...
{		
	...
	$sql = "SELECT id FROM fabric WHERE line=\"Kona Cotton\"";
	
	$konaresult = mysqli_query($con, $sql) or die (mysqli_error($con)); // run the query and assign the result to $konaresult

	// go through each Kona - id to color in fabriccolor
	while ($kona = mysqli_fetch_array($konaresult))
	{
		$id = $kona['id'];
	...
	foreach ($fabrics as $fabric)
	{
		...
		while ($row = mysqli_fetch_array($result)) {
			if (strcmp($row["line"], "Kona Cotton") == 0)
			{ 		
				$returns[] = $row["url"];
				$returns[] = $row["name"];
				$returns[] = $closestRGB;
